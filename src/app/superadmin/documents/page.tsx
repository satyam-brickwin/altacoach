...
export default function DocumentsPage() {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      let url = `/apisuper/documents?limit=${pagination.limit}&offset=${pagination.offset}`;
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/apisuper/documents', {
        method: 'POST',
        body: formData
      });
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/apisuper/documents/${selectedDocument.id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleProcessDocument = async (document: Document) => {
    try {
      const response = await fetch(`/apisuper/documents/${document.id}/process`, {
        method: 'POST'
      });
