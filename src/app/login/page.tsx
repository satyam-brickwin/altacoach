...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && !isLoading) {
      if (user?.role === UserRole.SUPER_ADMIN) {
        ...
        // Explicitly redirect STAFF and USER roles to /staff
        router.push('/staff');
      } else {
        router.push('/staff'); // Default to staff page instead of root
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // Instead of using redirect, use role-based routing
        switch (user?.role) {
